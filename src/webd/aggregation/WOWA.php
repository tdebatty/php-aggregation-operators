...
class WOWA {
    public static function wowa(array $w, array $p, array $a) {
        ...
        //$omega = new \web_d\vectors\Vector($omega);
        //$a = new \web_d\vectors\Vector($a);
...
function calculaSi(Point $di, Point $di_1) {
    if ($di->x == $di_1->x) {
        echo "Division by 0!\n";
        var_dump($di);
        var_dump($di_1);
...
function calculaMi($si, $siP1, Point $di, Point $di_1, Point $diP1) {
    if (($si * $siP1) < 0.0) {
        return 0.0;
    }
 ...
 * @param Point[] $points
 */
function calculaLi(&$lines, &$points) {
    $N = count($points);
    ...
    for ($i = 1; $i < $N; $i++) {
        $s[$i] = calculaSi($points[$i], $points[$i - 1]);
    }

    for ($i = 1; $i < ($N - 1); $i++) {
        $m[$i] = calculaMi($s[$i], $s[$i + 1], $points[$i], $points[$i - 1], $points[$i + 1]);
    }
...
class Funcio
{
    public $type = 1;    // int t     1: recta, 2: dobleBernstein 
